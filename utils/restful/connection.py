...
from ..common.exceptions import NotParseableException, NoResponseError
...
class RestfulRequest(Request):
    ...
    def _send(self, **kwargs):
        ...
        except self.engine_request_error as e:
    ...
    def _send_request_with_logging(self, **kwargs):
        logger = self.logger()
        logger.log_start(self, **kwargs)
        response = self._send_request(self.method, self.url, **kwargs)
        logger.log_end(self, response)
        return response
